...
if __name__ == "__main__":
    ...
    # -*- coding: utf-8 -*-
# EA · Ultra — All-in-One (KOR)
# Chat UI + Infinite Memory(JSONL) + Smart Retrieval + Skills(/use·/사용) + Mini-Autobuilder(/build·/제작)
...
APP_NAME   = "EA · Ultra (KOR AIO)"
BUILD_TAG  = "EA-ULTRA-20250818-KOR"
DATA_DIR   = Path("data"); DATA_DIR.mkdir(parents=True, exist_ok=True)
STATE_PATH = DATA_DIR / "state.json"
MEM_PATH   = DATA_DIR / "memory.jsonl"
DIALOG_LOG = DATA_DIR / "dialog.jsonl"
...
# ---------- Dynamic Skill Builder (버그픽스: f-string 미사용) ----------
DYN_DIR = DATA_DIR / "skills_dynamic"; DYN_DIR.mkdir(parents=True, exist_ok=True)

def make_dynamic_skill(name_slug:str, goal:str):
    """
    이름/설명만 받아 '순수 함수형' 스킬 클래스를 파일로 만들고 동적 import.
    내부 생성 코드에서 f-string을 쓰지 않고 문자열 결합으로만 작성해 SyntaxError 방지.
    """
    safe_slug = "".join(c if c.isalnum() or c in "._-" else "_" for c in name_slug.lower())
    modname   = f"dyn_{safe_slug.replace('.','_')}"
    clsname   = "".join(w.capitalize() for w in safe_slug.split(".")) + "Skill"
...
# ---------- Command Parsing (영/한 동시 지원) ----------
def is_cmd(text: str, *aliases: str) -> bool:
    text = (text or "").lstrip()
    return any(text.startswith(a+" ") or text.strip()==a for a in aliases)
...
def maybe_build_skill(user_text:str):
    ...
    if "|" in rest:
        name, desc = [x.strip() for x in rest.split("|",1)]
    else:
        name, desc = rest.strip(), rest.strip()
    try:
...
def render_app():
    st.set_page_config(page_title=APP_NAME, page_icon="✨", layout="centered")
    st.markdown(f"### {APP_NAME}")
    st.caption("단일 파일 · 무한 기억 · 하이브리드 검색 · 스킬(/use·/사용) · Autobuilder(/build·/제작)")
    ...
    # 입력
    user_text = st.chat_input("예) /사용 sample.echo 안녕  ·  /기억 !핀 중요 일정  ·  /제작 auto.단어수|단어/문자 수 세기")
    if user_text:
        user_text = dedupe(user_text.strip())
        add_msg(sget("session_id"), "user", user_text)
