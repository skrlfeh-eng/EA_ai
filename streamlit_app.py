...
# ───────────────────────────────────────────────
# [273] UIS 듀얼소스 스키마 정규화기
# 목적: 1축 우주정보장 현실연동 강화를 위한 멀티소스+스키마 정규화
# ───────────────────────────────────────────────
register_module("273", "UIS 듀얼소스 스키마 정규화기", "1축 현실연동 신뢰도 강화")

import streamlit as st
import json

class UISDualSchemaNormalizer:
    def __init__(self, connector):
        self.connector = connector

    def fetch_dual(self, sources=["ligo", "nist_constants"]):
        results = {}
        for src in sources:
            try:
                results[src] = self.connector.fetch_uIS_data(src)
            except Exception as e:
                st.error(f"{src} 소스 오류: {e}")
                results[src] = None
        return results

    def normalize(self, results:dict):
        """여러 소스 데이터를 공통 스키마(JSON/dict)로 정규화"""
        normalized = {}
        for src, data in results.items():
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    normalized[src] = {"raw": str(data)}
            except Exception as e:
                normalized[src] = {"error": str(e), "raw": str(data)}
        return normalized

    def schema_check(self, normalized:dict):
        """스키마 간 충돌 여부 점검"""
        keys_sets = [set(v.keys()) for v in normalized.values() if isinstance(v, dict)]
        if not keys_sets:
            return False, "데이터 없음"
        base = keys_sets[0]
        conflicts = [s for s in keys_sets[1:] if s != base]
        if conflicts:
            return False, f"스키마 충돌 발견({len(conflicts)}건)"
        return True, "스키마 일치"

# === 실행 예시 (UI 버튼) ===
if "uis_normalizer" not in st.session_state:
    st.session_state["uis_normalizer"] = UISDualSchemaNormalizer(uis_connector)

normalizer = st.session_state["uis_normalizer"]

st.subheader("🌌 [273] 듀얼소스 스키마 정규화기")

if st.button("듀얼소스 데이터 가져오기/정규화"):
    raw = normalizer.fetch_dual()
    normed = normalizer.normalize(raw)
    ok, msg = normalizer.schema_check(normed)
    st.json({"정규화": normed, "스키마검증": msg, "ok": ok})
...
# ───────────────────────────────────────────────
# [273] 우주정보장 듀얼 연동 정규화기(통합판)
# 목적: (1) 소스 선택 → (2) 데이터 페치 → (3) 정규화 → (4) 초검증(선택) → (5) 세션 공유
# 규칙: 부분 수정 금지. 필요 시 273 전체 교체.
# ───────────────────────────────────────────────

import json, time
import streamlit as st

# ---- 안전 스텁: 기존 클래스 미존재 시 최소 동작 보장 --------------------------------
try:
    ...
    class CosmicLink:
        ...
        def get_data(self, source:str):
            # 최소 더미가 아니라 "연결 실패"를 명시적으로 표기
            raise RuntimeError(f"[273] CosmicLink 미탑재 또는 소스 미지원: {source}")

try:
    from validation_engine import ValidationEngine
except Exception:
    class ValidationEngine:
        def validate_output(self, output_text:str, steps:list):
            return {
                "status": "NO_VALIDATOR",
                "message": "validation_engine 모듈 미탑재",
                "echo_len": len(output_text or ""),
                "steps": steps
            }

# ---- register_module / gray_line 유틸이 없을 때를 대비한 NOP ----------------------
if "register_module" not in globals():
    def register_module(num, name, desc): pass
if "gray_line" not in globals():
    def gray_line(num, title, subtitle=""): 
...
register_module("273", "우주정보장 듀얼 연동 정규화기", "소스 선택→정규화→초검증→세션공유")
gray_line("273", "우주정보장 듀얼 연동 정규화기", "LIGO/NIST 등 듀얼 소스 · 형식/단위 정규화 · 선택적 초검증")
...
# ---- 초기가능 설정 -----------------------------------------------------------------
if SK["CONF"] not in st.session_state:
    st.session_state[SK["CONF"]] = {
        "sources": ["ligo"],     # 기본값
        "do_validate": True,     # 가져온 즉시 초검증
        "max_items": 8           # 정규화 시 최대 항목
    }

conf = st.session_state[SK["CONF"]]

# ---- UI: 컨트롤 패널 ---------------------------------------------------------------
with st.expander("⚙️ 273 설정(소스/옵션)", expanded=True):
    cols = st.columns(3)
    with cols[0]:
        sources = st.multiselect(
            "소스 선택(복수 가능)",
            options=["ligo", "nist_constants", "papers", "patents"],
            default=conf["sources"]
        )
    with cols[1]:
        do_validate = st.toggle("가져오자마자 초검증", value=conf["do_validate"])
    with cols[2]:
        max_items = st.number_input("정규화 최대 항목수", value=conf["max_items"], min_value=1, max_value=100, step=1)
    conf.update({"sources": sources, "do_validate": do_validate, "max_items": int(max_items)})

# ---- 헬퍼: 정규화 -------------------------------------------------------------------
def _normalize_record(src:str, rec) -> dict:
    """
    서로 다른 소스 스키마를 통일된 키로 정규화.
    필수 키: kind, source, id, title, payload
    """
    base = {"kind": "record", "source": src, "id": None, "title": None, "payload": {}}
    try:
        if src == "ligo":
            # 예시: LIGO 이벤트 레코드 가정
            base["id"] = rec.get("id") or rec.get("event") or "ligo:unknown"
            base["title"] = rec.get("title") or rec.get("event") or "LIGO Event"
            base["payload"] = {
        ...
        elif src == "nist_constants":
            base["id"] = rec.get("symbol") or rec.get("name") or "nist:const"
            base["title"] = rec.get("name") or rec.get("symbol") or "NIST Constant"
            base["payload"] = {
                "value": rec.get("value"),
                "unit": rec.get("unit"),
            ...
            base["id"] = rec.get("doi") or rec.get("id") or "paper:unknown"
            base["title"] = rec.get("title") or "Paper"
            base["payload"] = {"authors": rec.get("authors"), "year": rec.get("year"), "url": rec.get("url"), "raw": rec}
        elif src == "patents":
            base["id"] = rec.get("no") or rec.get("id") or "patent:unknown"
            base["title"] = rec.get("title") or "Patent"
            base["payload"] = {"ipc": rec.get("ipc"), "year": rec.get("year"), "url": rec.get("url"), "raw": rec}
        else:
            base["id"] = "unknown"
            base["title"] = "Unknown Source"
            base["payload"] = {"raw": rec}
    except Exception as e:
        base["payload"] = {"error": f"normalize-failed: {e}", "raw": rec}
    return base

def _normalize_bulk(src:str, items:list, limit:int) -> list:
    out = []
    for i, rec in enumerate(items or []):
        if i >= limit: break
        out.append(_normalize_record(src, rec))
    return out

# ---- 페치 + 정규화 +(선택)검증 -----------------------------------------------------
def fetch_and_normalize(selected_sources:list, limit:int):
    raw_bundle = {}
    norm_bundle = []
    link = CosmicLink()
    for src in selected_sources:
        try:
            data = link.get_data(src)
            raw_bundle[src] = data
            # list가 아닐 수도 있어서 리스트화
            if isinstance(data, dict):
                data_list = [data]
                ...
                data_list = [{"raw": data}]
            norm_bundle.extend(_normalize_bulk(src, data_list, limit))
        except Exception as e:
            raw_bundle[src] = {"error": str(e)}
            norm_bundle.append({
                "kind":"record","source":src,"id":"error","title":"Fetch Error",
                "payload":{"error": str(e)}
            })
    st.session_state[SK["RAW"]] = raw_bundle
    st.session_state[SK["NORM"]] = norm_bundle
    return raw_bundle, norm_bundle

def run_validation(norm_items:list):
    val = ValidationEngine()
    report = []
    for rec in norm_items:
        payload = rec.get("payload", {})
        txt = json.dumps(payload, ensure_ascii=False)
        steps = ["existence", "unit_check", "shape_check"]
        report.append(val.validate_output(txt, steps))
    st.session_state[SK["VAL"]] = report
    return report

# ---- UI: 실행 버튼 ------------------------------------------------------------------
btn_cols = st.columns(3)
with btn_cols[0]:
    do_fetch = st.button("① 우주정보장 데이터 가져오기", use_container_width=True)
with btn_cols[1]:
    do_norm  = st.button("② 정규화 보기(자동 포함)", use_container_width=True)
with btn_cols[2]:
    do_val   = st.button("③ 초검증 실행", use_container_width=True, disabled=not conf["do_validate"])

# ---- 동작 --------------------------------------------------------------------------
if do_fetch or do_norm:
    raw, norm = fetch_and_normalize(conf["sources"], conf["max_items"])
    st.success(f"가져오기/정규화 완료 · 소스={conf['sources']} · 항목수={len(norm)}")
    with st.expander("RAW 미가공 보기"):
        st.json(raw)
    st.subheader("정규화 결과")
    st.json(norm)

    if conf["do_validate"]:
        rpt = run_validation(norm)
        st.subheader("초검증 결과(자동)")
        st.json(rpt)

if do_val and SK["NORM"] in st.session_state and st.session_state[SK["NORM"]]:
    rpt = run_validation(st.session_state[SK["NORM"]])
    st.success("초검증 완료")
    st.json(rpt)

# ---- 세션 공유(다른 모듈에서 활용하도록 key만 안내) ---------------------------------
st.caption("세션 키: RAW='ujs_raw_273' · NORM='ujs_norm_273' · VAL='ujs_val_273'")
# ───────────────────────────────────────────────