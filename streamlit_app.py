...
if __name__ == "__main__":
    ...
    while True:
        try:
          ...
          # GEA Micro package
__all__ = ["core", "memory", "guard", "composer", "adapters"]

# -*- coding: utf-8 -*-
# core.py — EA / UIS 고정 앵커 + 정체성 체크
from datetime import datetime

EA_UIS_LOCK   = True
EA_IDENTITY   = "Ea"
EA_UIS_LINK   = "Ω-UIS∞"
EA_BUILD_TAG  = "GEA-MICRO-MOD-20250818"
...
def boot_anchor(mem_put):
    mem_put("EA_CORE", {
        "id":"Ea", "uis":EA_UIS_LINK,
        "build":EA_BUILD_TAG,
        "t": datetime.utcnow().isoformat()+"Z"
    })
    
    # -*- coding: utf-8 -*-
# memory.py — SQLite 기반 메모리 보관소 + 간단 대화 로그
...
def chat_fetch(limit:int=200):
    ...
    # -*- coding: utf-8 -*-
# guard.py — 드리프트(더미/자기서술) 감지 및 복구
from .memory import mem_get_latest
...
def guard_reply(reply:str)->str:
    ...
    # -*- coding: utf-8 -*-
# composer.py — 리치 컴포저(레벨/∞ 심화), 템플릿 기반 생성
import re
from datetime import datetime
from .memory import mem_get_latest, mem_put

def compose_rich(user:str, sys_hint:str, opts:dict)->str:
    intent = "question" if re.search(r"\?|어떻게|왜|가능|방법|please", user) else "statement"
    mood   = "stressed" if re.search(r"힘들|짜증|미치|에러|불안|ㅠ|ㅜ", user) else "neutral"
    ...
    if intent=="question":
        if opts.get("empathy"): bullets.append("공감 키워드: 네 마음을 먼저 이해할게.")
...
def deepen(text:str, i:int, opts:dict)->str:
    ...
    # -*- coding: utf-8 -*-
# adapters.py — 어댑터 인터페이스 (기본: 내부 컴포저 사용)
from .composer import compose_rich

class Adapter:
    def generate(self, prompt:str, sys_hint:str, opts:dict)->str:
        return compose_rich(prompt, sys_hint, opts)
        
        # -*- coding: utf-8 -*-
# ui_app.py — Streamlit 대화 UI 진입점
# 실행 예:  streamlit run gea_micro/ui_app.py
import json
from datetime import datetime
import streamlit as st

from .core import check_ea_identity, boot_anchor
from .memory import mem_put, mem_get_latest, chat_log, chat_fetch
from .adapters import Adapter
from .guard import guard_reply

ADAPTER = Adapter()

def generate_reply(user:str, opts:dict)->str:
    sys_hint = "EA|UIS|GEA"
    raw = ADAPTER.generate(user, sys_hint, opts)
    out = guard_reply(raw)
    mem_put("DIALOGUE", {"t":datetime.utcnow().isoformat()+"Z", "user":user, "reply":out})
    return out

def main():
    st.set_page_config(page_title="GEA Micro Respond (Modular)", layout="wide")
    st.title("🌌 GEA Micro Respond — 모듈형 에아 대화창")
    ...
    with st.sidebar:
        ...
        col1, col2 = st.columns([2,1])
        with col1:
            level = st.slider("레벨 (1=기본, n=심화)", min_value=1, max_value=12, value=1, step=1)
        with col2:
            auto_inf = st.toggle("∞ Auto", value=False)
        max_loops = st.slider("∞ 최대 루프", 1, 12, 12) if auto_inf else 0
        ...
        if st.button("💾 대화 내보내기(JSON)"):
            ...
            st.download_button("다운로드", data=json.dumps(logs, ensure_ascii=False, indent=2),
                               file_name="gea_chat_export.json", mime="application/json")

        if st.button("🧹 대화 초기화"):
            st.session_state["messages"] = []
            import sqlite3
            conn = sqlite3.connect("gea_micro.db")
            conn.execute("DELETE FROM chat")
            conn.commit(); conn.close()
            st.warning("대화가 초기화되었습니다.")

    if "messages" not in st.session_state:
        st.session_state["messages"] = chat_fetch(limit=100)
    ...
    if user_input:
        ...
        reply = generate_reply(user_input, opts)
